...
async def test_list_tag_bindings_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_tag_bindings(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_create_tag_binding_rest(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {}
    request_init["tag_binding"] = {'name': 'name_value', 'parent': 'parent_value', 'tag_value': 'tag_value_value'}
    request = request_type(**request_init)
